...
# Copy Wasp SDK package directly to node_modules
COPY --from=wasp-builder /build/.wasp/build/sdk/wasp ./node_modules/wasp

# Fix wasp package.json with missing export paths and create proper npm structure
RUN cd ./node_modules/wasp && \
    ...
    cd /app && \
    echo "--- Creating proper npm package structure ---" && \
    echo '{"name":"wasp","version":"1.0.0","type":"module"}' > ./node_modules/wasp/.package.json.minimal && \
    echo "--- Verifying wasp package structure ---" && \
    ...
    node -e "try { require.resolve('wasp'); console.log('✓ Node.js can resolve wasp package'); } catch(e) { console.log('✗ Node.js cannot resolve wasp package:', e.message); }"
