...
export const DATABASE_CONFIG = {
  ...
  DEFAULT_ISOLATION_LEVEL: 'ReadCommitted' as const,
  CREDIT_ISOLATION_LEVEL: 'ReadCommitted' as const,
...
export async function withDatabaseRetry<T>(
  ...
  delayMs: number = DATABASE_CONFIG.RETRY_DELAY_MS
): Promise<T> {
  let lastError: Error;
...
export class DatabaseMetrics {
  ...
  recordQuery(queryName: string, durationMs: number): void {
    if (!DATABASE_CONFIG.ENABLE_QUERY_LOGGING) return;
